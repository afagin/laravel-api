...
class InvoicesGenerator
{
    ...
    public function generateApplicationUserInvoice($applicationUser_id, $order_id)
    {
        ...
        if ($orderInfo->payInId_share_bill == null) {
            ...
            $pdf->save(storage_path('tmp/') . $invoice_id . '.pdf');

            $disk = $disk = Storage::disk('gcs');
            $disk->put(Config::get('constants.applicationUsers_invoices_bucket') . $invoice_id . '.pdf', file_get_contents(storage_path('tmp/' . $invoice_id . '.pdf')));
            $disk->setVisibility(Config::get('constants.applicationUsers_invoices_bucket') . $invoice_id . '.pdf', 'public');
            unlink(storage_path('tmp/' . $invoice_id . '.pdf'));
    ...
    public function generatePartnerInvoice($start_date, $end_date, $partner_id)
    {
        ...
        $disk = $disk = Storage::disk('gcs');
        $disk->put(Config::get('constants.partners_invoice_bucket') . $invoice_id . '.pdf', file_get_contents(storage_path('tmp/' . $invoice_id . '.pdf')));
        $disk->setVisibility(Config::get('constants.partners_invoice_bucket') . $invoice_id . '.pdf', 'public');
        unlink(storage_path('tmp/' . $invoice_id . '.pdf'));
        return [
            'invoice_id' => $invoice_id,
