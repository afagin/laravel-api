...
class PartnerController extends Controller
{
    ...
    )
    {
        ...
        if ($request->hasFile('picture')) {
            ...
            $disk->putFileAs(Config::get('constants.partners_img_bucket'), $request->file('picture'), $filename);
            $disk->setVisibility(Config::get('constants.partners_img_bucket') . $filename, 'public');
        }
    ...
    )
    {
        ...
        $picture_url = Storage::disk('gcs')->url(Config::get('constants.partners_img_bucket') . $partner->picture);
        return view('partners.edit', compact('partner', 'partnerCategories', 'picture_url'));
    }
    ...
    )
    {
        ...
        if ($request->hasFile('picture')) {
            ...
            $filename = time() . '.' . $picture->getClientOriginalExtension();
            $partner = $this->partner->findOrFail($partner_id);
            if (isset($partner->picture) && !empty($partner->picture)) {
                $disk = $disk = Storage::disk('gcs');
                $disk->move(Config::get('constants.partners_img_bucket') . $partner->picture, Config::get('constants.trash_bucket') . $partner->picture);
            }
            $disk = Storage::disk('gcs');
            $disk->putFileAs(Config::get('constants.partners_img_bucket'), $request->file('picture'), $filename);
            $disk->setVisibility(Config::get('constants.partners_img_bucket') . $filename, 'public');
