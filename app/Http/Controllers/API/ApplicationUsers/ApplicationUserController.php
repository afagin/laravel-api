...
class ApplicationUserController extends Controller
{
    ...
    )
    {
        ...
        if ($request->hasFile('picture')) {
            ...
            $disk = $disk = Storage::disk('gcs');
            $disk->putFileAs(Config::get('constants.applicationUsers_img_bucket'), $request->file('picture'), $filename);
            $disk->setVisibility(Config::get('constants.applicationUsers_img_bucket') . $filename, 'public');
        } else {
            $filename = null;
        }
        ...
        if (isset($request['base64']) && $request['base64'] !== null) {
            ...
            file_put_contents(storage_path('tmp/' . $filename), $picture);
            $disk = $disk = Storage::disk('gcs');
            $disk->put(Config::get('constants.applicationUsers_img_bucket') . $filename, file_get_contents(storage_path('tmp/' . $filename)));
            $disk->setVisibility(Config::get('constants.applicationUsers_img_bucket') . $filename, 'public');
    ...
    )
    {
        ...
        if (isset($user['picture']) && !empty($user['picture'])) {
            $user['picture'] = $picture_url = Storage::disk('gcs')->url(Config::get('constants.applicationUsers_img_bucket') . $user['picture']);;
        }
    ...
    )
    {
        ...
        if ($request->hasFile('picture')) {
            $picture = $request->file('picture');
            $filename = time() . '.' . $picture->getClientOriginalExtension();
            if (isset($partner->picture) && !empty($applicationUser->picture)) {
                $disk = $disk = Storage::disk('gcs');
                $disk->move(Config::get('constants.applicationUsers_img_bucket') . $applicationUser->picture, Config::get('constants.trash_bucket') . $applicationUser->picture);
            }
        ...
        if (isset($request['base64']) && $request['base64'] !== null) {
            ...
            $disk->setVisibility(Config::get('constants.applicationUsers_img_bucket') . $filename, 'public');
            unlink(storage_path('tmp/' . $filename));
            if (isset($applicationUser->picture) && !empty($applicationUser->picture)) {
                $disk = $disk = Storage::disk('gcs');
                $disk->move(Config::get('constants.applicationUsers_img_bucket') . $applicationUser->picture, Config::get('constants.trash_bucket') . $applicationUser->picture);
            }
        }
